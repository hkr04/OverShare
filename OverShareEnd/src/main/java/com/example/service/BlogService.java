...
public class BlogService {
    ...
    /**
     * 函数接受一个Integer类型的参数blogId，表示要获取推荐作品的相关作品ID。
     * 首先通过调用类中的selectById(blogId)方法，根据传入的blogId查询并获取对应作品对象，并将其赋值给Blog类型变量blog。
     * 从获取到的blog对象中调用getTags()方法获取此作品的标签信息，并存储在字符串变量tags中。
     * 初始化一个HashSet类型的blogSet，用于存放满足条件的推荐作品对象。
     * 使用ObjectUtil.isNotEmpty(tags)判断作品是否有标签信息，如果有，则执行以下操作：
     * a. 调用selectAll(null)方法获取数据库中所有作品对象组成的列表。
     * b. 使用JSON工具库（这里是JSONUtil）解析字符串形式的标签信息为JSONArray对象，即tagsArr。
     * c. 遍历这个JSONArray对象，对于其中每一个标签（以tag表示），执行以下筛选逻辑：
     * i. 应用Java 8流(Stream)对所有作品列表进行过滤(filter)操作，筛选出那些其标签集合中含有当前遍历到的标签（将tag转换为字符串）的作品对象。
     * ii. 将过滤后的作品对象集合转换为一个新的HashSet，并使用collect(Collectors.toSet())方法收集这些对象。
     * iii. 将上述收集到的包含特定标签的作品HashSet添加到总的推荐作品集合blogSet中。
     * 在完成上述循环后，blogSet中包含了所有与指定作品具有相同标签的其他作品。
     * 最终，函数返回填充了推荐作品对象的blogSet集合。
     *
     * @param blogId
     * @return
     */
